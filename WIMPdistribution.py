...
def integrand3d(w, xi, x, M, m):
    ...
    if xi*w < 1.:
      ...
      if z>x: 
        ...
        if xm < 1.:
          arc = np.arctanh(1./xm)
          a   = 4.*xi/x*ffv*rhotot(xi,M,m)/z**1.5*arc
    return a
...
def rhoF3dn(x, M, m):
    ...
    xc = x*np.geomspace(eps, 1., NT)
    for i, xi in enumerate(xc):
      w    = np.linspace(x/xi/(x+xi), 1./xi, NT)
      ll[i] = np.trapz(integrand3d(w, xi, x, M, m), w)
    a  = np.trapz(ll, xc)
    # Integration over the region xi>x
    xc = x*np.geomspace(1., bigN, NT)
    for i, xi in enumerate(xc):
      w    = np.linspace(eps, x/xi/(x+xi), NT)
      ll[i] = np.trapz(integrand3d(w, xi, x, M, m), w)
    a  = a + np.trapz(ll, xc)
...
def Gamma3d(M, m):
    ...
    a  = xmax(M, m)
    xt = np.geomspace(1.1e-10*a, 1.e20*a, 30)
...
mt = 0.1
a  = xmax(Mt, mt)
xt = np.geomspace(1.e-6*a, 1.e15*a, 25)
zt = rg(Mt)*xt
rt = rhoF3d(xt, Mt, mt)
plt.loglog(zt, rt, 'k-')
plt.show()
